// Chunk meshing: face culling, directional shading, vertex AO, smooth per-vertex
// sky/torch light (separate attributes so the shader can scale skylight with the
// time of day). Also handles cross-rendered blocks (plants, torches).

import * as THREE from 'three';
import { AIR, BLOCKS, isOpaque } from './blocks.js';
import { tileUV } from '../render/textures.js';
...
class GeoBuffer {
    ...
    build() {
        ...
        g.setAttribute('aColor', new THREE.Float32BufferAttribute(this.colors, 1));
        g.setAttribute('uv', new THREE.Float32BufferAttribute(this.uvs, 2));
        g.setAttribute('aSky', new THREE.Float32BufferAttribute(this.sky, 1));
        g.setAttribute('aTorch', new THREE.Float32BufferAttribute(this.torch, 1));
            ...
            for (let y = 0; y < HEIGHT; y++) {
                ...
                for (const face of FACES) {
                    ...
                    const nx = wx + dx,
                        ny = y + dy,
                        nz = wz + dz;
                    if (!faceVisible(id, world.getBlock(nx, ny, nz))) continue;
                    ...
                    for (let i = 0; i < 4; i++) {
                        ...
                        // sample the 4 cells touching this vertex in the layer outside the face
                        ...
                        for (let k = 0; k < 4; k++) {
                            ...
                            if (k > 0 && isOpaque(world.getBlock(sx, sy, sz))) {
                                if (k < 3) aoCount++;
                                else if (aoCount < 2) aoCount++; // corner only counts if sides open
                                continue;
                            }
...
// Small standalone cube geometry for item drops / held block, fully sky-lit.
